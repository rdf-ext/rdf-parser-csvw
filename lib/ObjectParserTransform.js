...
class ObjectParserTransform extends Transform {
  constructor (options) {
    ...
    this.tableSchema = new TableSchema(this.factory, this.metadata, null, this.baseIRI)
  ...
  processTable () {
    ...
    if (this.metadata) {
      const urlQuad = this.metadata.match(null, this.factory.namedNode('http://www.w3.org/ns/csvw#url')).toArray().shift()

      if (urlQuad) {
        this.copySubgraph(this.metadata.match(urlQuad.subject).filter((quad) => {
  ...
  copySubgraph (quads, subject) {
    quads.forEach((quad) => {
      ...
      if (quad.object.termType === 'BlankNode') {
        this.copySubgraph(this.metadata.match(quad.object))
      }
    })
  }
}
