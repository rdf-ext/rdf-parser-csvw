  ...
  value (column, row) {
    ...
    if (column.datatype.base.value === this.ns.dateTime.value) {
      if (column.datatype.format) {
        const dateTime = DateTime.fromFormat(value, column.datatype.format, { zone: this.timezone })

        return this.factory.literal(dateTime.toISO(), this.ns.dateTime)
      }

      return this.factory.literal((new Date(value)).toISOString(), this.ns.dateTime)
    }
