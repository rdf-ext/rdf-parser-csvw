...
class TableSchema {
  constructor (dataset, { root, baseIRI, factory, timezone } = {}) {
    ...
    if (this.dataset) {
      this.aboutUrl = this.parseUrl(this.ns.aboutUrl) || this.aboutUrl
      this.propertyUrl = { fill: this.parseUrl(this.ns.propertyUrl) }
      this.parseColumns()
    }
  }

  parseUrl (property) {
    const url = RdfUtils.findValue(this.dataset, this.root, property)

    if (!url) {
      return
    }

    const urlTemplate = uriTemplate(url)

    return (row) => {
      return this.factory.namedNode(URL.resolve(this.baseIRI, urlTemplate.fill(row))) // eslint-disable-line node/no-deprecated-api
    }
  }
  ...
  createAllColumns (row) {
    ...
    const undefinedColumns = difference(Object.keys(row), titles).map((titles, title) => {
      return {
        name: title,
        titles: [title],
