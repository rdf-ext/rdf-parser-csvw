...
class TableSchema {
  constructor (dataset, { root, baseIRI, factory, timezone, table, tableGroup } = {}) {
    ...
    if (this.dataset) {
      ...
      this.nullValues = this.parseNullValues()
      this.parseColumns()
    }
  }
  ...
  parseNullValues () {
    const tableGroupNullValues = this.tableGroup ? RdfUtils.findValues(this.dataset, this.tableGroup, this.ns.null) : []
    const tableNullValues = RdfUtils.findValues(this.dataset, this.table, this.ns.null)
    const tableSchemaNullValues = RdfUtils.findValues(this.dataset, this.root, this.ns.null)

    return [...tableGroupNullValues, ...tableNullValues, ...tableSchemaNullValues, '']
  }
  ...
  parseColumns () {
    ...
    this.parsedColumns = RdfUtils.parseArray(this.dataset, columnNode).map((node) => {
      ...
      const nullValues = [...RdfUtils.findValues(this.dataset, node, this.ns.null), ...this.nullValues]
  ...
  value (column, row) {
    ...
    if (typeof value === 'undefined' || column.nullValues.includes(value)) {
      return undefined
    }
